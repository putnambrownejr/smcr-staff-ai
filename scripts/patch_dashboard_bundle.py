...
PATCHES: list[tuple[str, ...]] = [
    ...
    (
        ...
        # Stable marker: a later patch rewrites the scores literal that used to
        # live in this patch's `new` text, so `new` can no longer be its own
        # marker. The pending-id line is unique to the patched state.
        '      const tempId = "pending-" + Date.now();\n',
        "  newFitrep() {\n"
        "    return () => {\n"
        "      const id = Date.now();\n"
    ...
    (
        'template viewer: keep template id in bench library items',
        '      const templateItems = (data.template_library || []).filter((item) => item.source_path).map((item) => ({\n        name: item.template_name,\n        meta: item.template_source === "system" ? "system" : "saved",\n        path: item.source_path,\n      }));\n',
        '      const templateItems = (data.template_library || []).filter((item) => item.source_path).map((item) => ({\n        name: item.template_name,\n        meta: item.template_source === "system" ? "system" : "saved",\n        path: item.source_path,\n        templateId: item.template_id,\n      }));\n',
    ),
    (
        'template viewer: add templateViewer state',
        '    benchModal: null,\n    staffLaneModal: null,\n',
        '    benchModal: null,\n    templateViewer: null,\n    staffLaneModal: null,\n',
    ),
    (
        'template viewer: add viewer methods and bindings helper',
        '  _templateViewerBindings() {\n',
        '  benchAddDraft(cardIdx, itemIdx) {\n',
        '  openTemplateViewer(templateId, templateName) {\n    return () => {\n      if (!templateId) return;\n      this.setState({ benchModal: null, templateViewer: { id: templateId, name: templateName || "Template", loading: true, error: "", detail: null, showExample: false, copied: false } });\n      this._loadTemplateDetail(templateId);\n    };\n  }\n  async _loadTemplateDetail(templateId) {\n    try {\n      const res = await fetch("/product-templates/system/" + encodeURIComponent(templateId), { headers: this._apiHeaders() });\n      if (!res.ok) throw new Error("load failed");\n      const detail = await res.json();\n      this.setState((s) => (s.templateViewer && s.templateViewer.id === templateId ? { templateViewer: { ...s.templateViewer, loading: false, detail } } : null));\n    } catch (_) {\n      this.setState((s) => (s.templateViewer && s.templateViewer.id === templateId ? { templateViewer: { ...s.templateViewer, loading: false, error: "This template could not be loaded. Check that the local server is running." } } : null));\n    }\n  }\n  closeTemplateViewer() {\n    return () => this.setState({ templateViewer: null });\n  }\n  toggleTemplateExample() {\n    return () => this.setState((s) => (s.templateViewer ? { templateViewer: { ...s.templateViewer, showExample: !s.templateViewer.showExample } } : null));\n  }\n  copyTemplatePrompt() {\n    return () => {\n      const view = this.state.templateViewer;\n      if (!view || !view.detail) return;\n      const d = view.detail;\n      const lines = [];\n      lines.push("Draft a " + d.template_name + " with me.");\n      lines.push("");\n      if (d.when_to_use) { lines.push("When this product is used: " + String(d.when_to_use).trim()); lines.push(""); }\n      lines.push("Work through this structure with me. For each section, follow the guidance and help me replace it with real, reviewed content:");\n      lines.push("");\n      (d.sections || []).forEach((section) => {\n        lines.push("## " + section.heading);\n        lines.push(String(section.scaffold || "").trim());\n        lines.push("");\n      });\n      if ((d.guidance || []).length) {\n        lines.push("Keep this guidance in mind:");\n        (d.guidance || []).forEach((g) => lines.push("- " + g));\n        lines.push("");\n      }\n      if ((d.warnings || []).length) {\n        lines.push("Constraints:");\n        (d.warnings || []).forEach((w) => lines.push("- " + w));\n      }\n      const mark = (result) => this.setState((s) => (s.templateViewer ? { templateViewer: { ...s.templateViewer, copied: result } } : null));\n      if (navigator.clipboard && navigator.clipboard.writeText) {\n        navigator.clipboard.writeText(lines.join("\\n")).then(() => mark("ok"), () => mark("fail"));\n      } else {\n        mark("fail");\n      }\n    };\n  }\n  _templateViewerBindings() {\n    const view = this.state.templateViewer;\n    const detail = view && view.detail ? view.detail : null;\n    const showExample = !!(view && view.showExample);\n    const sections = detail && detail.sections ? detail.sections : [];\n    const asRows = (list) => (list || []).map((text) => ({ text }));\n    return {\n      templateViewerOpen: !!view,\n      templateViewerName: view ? view.name : "",\n      templateViewerLoading: !!(view && view.loading),\n      templateViewerError: view ? view.error : "",\n      templateViewerOneLiner: detail ? (detail.one_liner || "") : "",\n      templateViewerWhenToUse: detail ? (detail.when_to_use || "") : "",\n      templateViewerHasDetail: !!detail,\n      templateViewerModeLabel: showExample ? "Showing a worked example. It is notional \\u2014 replace every value with reviewed content." : "Showing the annotated scaffold: what belongs in each section.",\n      templateViewerToggleLabel: showExample ? "Show annotated scaffold" : "Show worked example",\n      templateViewerCopyLabel: view && view.copied === "ok" ? "Drafting prompt copied" : (view && view.copied === "fail" ? "Copy blocked — select the text and copy" : "Copy drafting prompt"),\n      templateViewerAgentLabel: detail && detail.agent_handoff ? "Drafts with the " + detail.agent_handoff + " agent" : "",\n      templateViewerSections: sections.map((section) => ({\n        heading: section.heading,\n        body: showExample ? String(section.example || "(No worked example for this section.)").trim() : String(section.scaffold || "").trim(),\n      })),\n      templateViewerGuidance: asRows(detail && detail.guidance),\n      templateViewerWarnings: asRows(detail && detail.warnings),\n      templateViewerDoctrine: asRows(detail && detail.doctrine_refs),\n      closeTemplateViewer: this.closeTemplateViewer(),\n      toggleTemplateExample: this.toggleTemplateExample(),\n      copyTemplatePrompt: this.copyTemplatePrompt(),\n    };\n  }\n  benchAddDraft(cardIdx, itemIdx) {\n',
    ),
    (
        'template viewer: expose viewer bindings',
        '      benchModalTitle, benchModalCard, benchModalNote, benchModalActions,\n',
        '      benchModalTitle, benchModalCard, benchModalNote, benchModalActions,\n      ...this._templateViewerBindings(),\n',
    ),
    (
        'template viewer: open viewer from the bench modal',
        '        benchModalNote = item.meta === "system"\n          ? `System template source: ${item.path} — open the location to view the template, or start a new draft copied into your Project files (loose, until you sort it into a project).`\n          : `Saved draft: ${item.path} — open it at that location.`;\n        benchModalActions = item.meta === "system"\n          ? [\n              { label: "Start new draft", style: benchActionStyle(true), onClick: this.benchAddDraft(cardIdx, itemIdx) },\n              { label: "Open template location", style: benchActionStyle(false), onClick: this.openFileLocation(item.path || "") },\n            ]\n          : [\n              { label: "Open file location", style: benchActionStyle(true), onClick: this.openFileLocation(item.path || "") },\n            ];\n',
        '        benchModalNote = item.meta === "system"\n          ? "Curated system template. Open the viewer for the annotated scaffold and a worked example, or start a new draft copied into your Project files (loose, until you sort it into a project)."\n          : `Saved draft: ${item.path} — open it at that location.`;\n        benchModalActions = item.meta === "system"\n          ? [\n              { label: "View template", style: benchActionStyle(true), onClick: this.openTemplateViewer(item.templateId, item.name) },\n              { label: "Start new draft", style: benchActionStyle(false), onClick: this.benchAddDraft(cardIdx, itemIdx) },\n            ]\n          : [\n              { label: "Open file location", style: benchActionStyle(true), onClick: this.openFileLocation(item.path || "") },\n            ];\n',
    ),
    (
        'template viewer: render viewer modal',
        '    <sc-if value="{{ benchModalOpen }}" hint-placeholder-val="{{ false }}">\n',
        '    <!-- ==================== TEMPLATE VIEWER MODAL ==================== -->\n    <sc-if value="{{ templateViewerOpen }}" hint-placeholder-val="{{ false }}">\n    <div style="position:fixed;inset:0;z-index:45;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;padding:24px;" sc-camel-on-click="{{ closeTemplateViewer }}">\n      <div sc-camel-on-click="{{ stopClickPropagation }}" style="width:min(780px,94vw);max-height:86vh;overflow:auto;border:1px solid #313844;border-radius:8px;background:#12161b;box-shadow:0 24px 60px rgba(0,0,0,0.55);padding:22px;">\n        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:12px;">\n          <div>\n            <div style="font-size:0.7rem;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:#8a94a0;">Template library</div>\n            <h3 style="margin:4px 0 0;font-size:1.14rem;font-weight:700;">{{ templateViewerName }}</h3>\n            <p style="margin:6px 0 0;color:#aab4bf;font-size:0.84rem;line-height:1.5;">{{ templateViewerOneLiner }}</p>\n          </div>\n          <button type="button" sc-camel-on-click="{{ closeTemplateViewer }}" aria-label="Close template viewer" style="background:transparent;border:0;color:#8a94a0;font-size:1.1rem;cursor:pointer;padding:0;line-height:1;">✕</button>\n        </div>\n        <sc-if value="{{ templateViewerLoading }}" hint-placeholder-val="{{ false }}">\n          <p style="margin:0 0 12px;color:#8a94a0;font-size:0.82rem;">Loading template…</p>\n        </sc-if>\n        <sc-if value="{{ templateViewerError }}" hint-placeholder-val="{{ false }}">\n          <p style="margin:0 0 12px;color:#d8a0a5;font-size:0.82rem;">{{ templateViewerError }}</p>\n        </sc-if>\n        <sc-if value="{{ templateViewerHasDetail }}" hint-placeholder-val="{{ false }}">\n        <div>\n          <sc-if value="{{ templateViewerWhenToUse }}" hint-placeholder-val="{{ false }}">\n            <div style="border-left:2px solid #d6bd7a;background:#0d1014;padding:9px 12px;margin-bottom:12px;">\n              <strong style="display:block;font-size:0.74rem;text-transform:uppercase;letter-spacing:0.05em;color:#d6bd7a;margin-bottom:4px;">When to use</strong>\n              <p style="margin:0;color:#aab4bf;font-size:0.8rem;line-height:1.5;">{{ templateViewerWhenToUse }}</p>\n            </div>\n          </sc-if>\n          <div style="display:flex;gap:9px;flex-wrap:wrap;align-items:center;margin-bottom:8px;">\n            <button type="button" sc-camel-on-click="{{ toggleTemplateExample }}" style="height:32px;padding:0 12px;border:1px solid #b21f2d;border-radius:6px;background:#b21f2d;color:#f5ebe9;font:inherit;font-weight:600;font-size:0.76rem;cursor:pointer;">{{ templateViewerToggleLabel }}</button>\n            <button type="button" sc-camel-on-click="{{ copyTemplatePrompt }}" style="height:32px;padding:0 12px;border:1px solid #313844;border-radius:6px;background:#1a2027;color:#eef2f6;font:inherit;font-weight:600;font-size:0.76rem;cursor:pointer;">{{ templateViewerCopyLabel }}</button>\n            <span style="color:#8a94a0;font-size:0.72rem;">{{ templateViewerAgentLabel }}</span>\n          </div>\n          <p style="margin:0 0 12px;color:#8a94a0;font-size:0.72rem;line-height:1.4;">{{ templateViewerModeLabel }}</p>\n          <div style="display:grid;gap:10px;">\n            <sc-for list="{{ templateViewerSections }}" as="sec" hint-placeholder-count="3">\n              <div style="border:1px solid #313844;border-radius:6px;background:#0d1014;padding:11px 13px;">\n                <strong style="display:block;font-size:0.84rem;font-weight:700;margin-bottom:5px;">{{ sec.heading }}</strong>\n                <p style="margin:0;white-space:pre-wrap;color:#aab4bf;font-size:0.79rem;line-height:1.5;">{{ sec.body }}</p>\n              </div>\n            </sc-for>\n          </div>\n          <div style="margin-top:14px;display:grid;gap:10px;">\n            <div>\n              <strong style="display:block;font-size:0.74rem;text-transform:uppercase;letter-spacing:0.05em;color:#8a94a0;margin-bottom:5px;">Guidance</strong>\n              <ul style="margin:0;padding-left:18px;color:#aab4bf;font-size:0.78rem;line-height:1.5;">\n                <sc-for list="{{ templateViewerGuidance }}" as="g" hint-placeholder-count="2"><li>{{ g.text }}</li></sc-for>\n              </ul>\n            </div>\n            <div>\n              <strong style="display:block;font-size:0.74rem;text-transform:uppercase;letter-spacing:0.05em;color:#8a94a0;margin-bottom:5px;">Doctrine references</strong>\n              <ul style="margin:0;padding-left:18px;color:#aab4bf;font-size:0.78rem;line-height:1.5;">\n                <sc-for list="{{ templateViewerDoctrine }}" as="d" hint-placeholder-count="2"><li>{{ d.text }}</li></sc-for>\n              </ul>\n            </div>\n            <div>\n              <strong style="display:block;font-size:0.74rem;text-transform:uppercase;letter-spacing:0.05em;color:#d6bd7a;margin-bottom:5px;">Warnings</strong>\n              <ul style="margin:0;padding-left:18px;color:#d6bd7a;font-size:0.78rem;line-height:1.5;">\n                <sc-for list="{{ templateViewerWarnings }}" as="w" hint-placeholder-count="2"><li>{{ w.text }}</li></sc-for>\n              </ul>\n            </div>\n          </div>\n        </div>\n        </sc-if>\n      </div>\n    </div>\n    </sc-if>\n    <sc-if value="{{ benchModalOpen }}" hint-placeholder-val="{{ false }}">\n',
    ),
    (
        'fitrep traits: grade the real 14 attributes, not the 5 sections',
        '  static FR_LEGACY_SECTIONS = {\n',
        '  static FR_TRAITS = [\n    { id: "mission", label: "A. Mission Accomplishment" },\n    { id: "individual", label: "B. Individual Character" },\n    { id: "leadership", label: "C. Leadership" },\n    { id: "intellect", label: "D. Intellect & Wisdom" },\n    { id: "fitness", label: "E. Fulfillment of Eval Duties" },\n  ];\n',
        '  static FR_TRAITS = [\n    { id: "performance", section: "D. Mission Accomplishment", label: "Performance" },\n    { id: "proficiency", section: "D. Mission Accomplishment", label: "Proficiency" },\n    { id: "courage", section: "E. Individual Character", label: "Courage" },\n    { id: "effectiveness", section: "E. Individual Character", label: "Effectiveness Under Stress" },\n    { id: "initiative", section: "E. Individual Character", label: "Initiative" },\n    { id: "leadingSubordinates", section: "F. Leadership", label: "Leading Subordinates" },\n    { id: "developingSubordinates", section: "F. Leadership", label: "Developing Subordinates" },\n    { id: "settingExample", section: "F. Leadership", label: "Setting the Example" },\n    { id: "ensuringWellbeing", section: "F. Leadership", label: "Ensuring Well-being of Subordinates" },\n    { id: "communication", section: "F. Leadership", label: "Communication Skills" },\n    { id: "pme", section: "G. Intellect and Wisdom", label: "Professional Military Education (PME)" },\n    { id: "decisionMaking", section: "G. Intellect and Wisdom", label: "Decision Making Ability" },\n    { id: "judgment", section: "G. Intellect and Wisdom", label: "Judgment" },\n    { id: "evaluations", section: "H. Fulfillment of Evaluation Responsibilities", label: "Evaluations" },\n  ];\n\n  // Reports saved before the tracker graded real attributes stored one value\n  // per SECTION. Spread each legacy section value across the attributes that\n  // section actually contains so old reports still open with sane marks.\n  static FR_LEGACY_SECTIONS = {\n    mission: ["performance", "proficiency"],\n    individual: ["courage", "effectiveness", "initiative"],\n    leadership: ["leadingSubordinates", "developingSubordinates", "settingExample", "ensuringWellbeing", "communication"],\n    intellect: ["pme", "decisionMaking", "judgment"],\n    fitness: ["evaluations"],\n  };\n\n  static frScores(raw) {\n    const src = raw || {};\n    const out = {};\n    Component.FR_TRAITS.forEach((t) => {\n      out[t.id] = typeof src[t.id] === "number" ? src[t.id] : null;\n    });\n    Object.keys(Component.FR_LEGACY_SECTIONS).forEach((legacyKey) => {\n      const value = src[legacyKey];\n      if (typeof value !== "number") return;\n      Component.FR_LEGACY_SECTIONS[legacyKey].forEach((id) => {\n        if (out[id] === null) out[id] = value;\n      });\n    });\n    Component.FR_TRAITS.forEach((t) => {\n      if (out[t.id] === null) out[t.id] = 4;\n    });\n    return out;\n  }\n',
    ),
    (
    ...
    (
        'fitrep traits: new report defaults to 14 attributes',
        'scores: { mission: 4, individual: 4, leadership: 4, intellect: 4, fitness: 4 }',
        'scores: { performance: 4, proficiency: 4, courage: 4, effectiveness: 4, initiative: 4, leadingSubordinates: 4, developingSubordinates: 4, settingExample: 4, ensuringWellbeing: 4, communication: 4, pme: 4, decisionMaking: 4, judgment: 4, evaluations: 4 }',
    ),
    (
        'fitrep traits: demo report 1 marks',
        'scores: { mission: 5, individual: 6, leadership: 5, intellect: 4, fitness: 6 }',
        'scores: { performance: 5, proficiency: 6, courage: 5, effectiveness: 5, initiative: 6, leadingSubordinates: 5, developingSubordinates: 4, settingExample: 6, ensuringWellbeing: 5, communication: 4, pme: 4, decisionMaking: 5, judgment: 5, evaluations: 6 }',
    ),
    (
        'fitrep traits: demo report 2 marks',
        'scores: { mission: 6, individual: 6, leadership: 6, intellect: 5, fitness: 5 }',
        'scores: { performance: 6, proficiency: 6, courage: 6, effectiveness: 6, initiative: 6, leadingSubordinates: 6, developingSubordinates: 5, settingExample: 6, ensuringWellbeing: 6, communication: 5, pme: 5, decisionMaking: 6, judgment: 5, evaluations: 5 }',
    ),
    (
        'fitrep traits: demo report 3 marks',
        'scores: { mission: 4, individual: 5, leadership: 4, intellect: 5, fitness: 4 }',
        'scores: { performance: 4, proficiency: 5, courage: 5, effectiveness: 4, initiative: 4, leadingSubordinates: 4, developingSubordinates: 3, settingExample: 5, ensuringWellbeing: 4, communication: 4, pme: 5, decisionMaking: 4, judgment: 5, evaluations: 4 }',
    ),
    (
