...
PATCHES: list[tuple[str, ...]] = [
    ...
    (
        ...
        "      </section>\n\n"
        '      <section style="border:1px solid #313844;border-radius:8px;background:#12161b;padding:18px;display:grid;gap:14px;">\n'
        '        <div><h3 style="margin:0 0 4px;font-size:1.02rem;font-weight:700;">Career Opportunities</h3><p style="margin:0;color:#8a94a0;font-size:0.82rem;">Official MARFORRES gateways for SMCR / IMA / ADOS opportunities. Listings live in browser-based portals — open a source to view current billets and apply.</p></div>\n'
        '        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:8px;"><sc-for list="{{ careerOpportunitySources }}" as="source" hint-placeholder-count="2"><div style="padding:12px;border:1px solid #313844;border-radius:6px;background:#0d1014;display:grid;gap:8px;align-content:start;"><strong style="font-size:0.82rem;">{{ source.name }}</strong><a href="{{ source.url }}" target="_blank" rel="noopener" style="color:#7db2e0;font-size:0.74rem;font-weight:600;">Open official source ↗</a></div></sc-for></div>\n'
        ...
        '        <form sc-camel-on-submit="{{ applyCareerOpportunityFilters }}" style="display:grid;grid-template-columns:repeat(4,minmax(130px,1fr));gap:8px;padding-top:4px;"><sc-raw-select value="{{ careerOpportunitySort }}" sc-camel-on-change="{{ onCareerOpportunitySort }}" aria-label="Sort career opportunities" style="height:34px;border:1px solid #313844;border-radius:6px;padding:0 8px;background:#0d1014;color:#eef2f6;font:inherit;"><sc-for list="{{ careerOpportunitySortOptions }}" as="option" hint-placeholder-count="4"><option value="{{ option.value }}">{{ option.label }}</option></sc-for></sc-raw-select><sc-raw-select value="{{ careerOpportunityDirection }}" sc-camel-on-change="{{ onCareerOpportunityDirection }}" aria-label="Sort direction" style="height:34px;border:1px solid #313844;border-radius:6px;padding:0 8px;background:#0d1014;color:#eef2f6;font:inherit;"><sc-for list="{{ careerOpportunityDirectionOptions }}" as="option" hint-placeholder-count="2"><option value="{{ option.value }}">{{ option.label }}</option></sc-for></sc-raw-select><sc-raw-select value="{{ careerOpportunityType }}" sc-camel-on-change="{{ onCareerOpportunityType }}" aria-label="Opportunity component" style="height:34px;border:1px solid #313844;border-radius:6px;padding:0 8px;background:#0d1014;color:#eef2f6;font:inherit;"><sc-for list="{{ careerOpportunityTypeOptions }}" as="option" hint-placeholder-count="4"><option value="{{ option.value }}">{{ option.label }}</option></sc-for></sc-raw-select><input value="{{ careerOpportunityKeyword }}" sc-camel-on-change="{{ onCareerOpportunityKeyword }}" placeholder="Keyword" aria-label="Keyword filter" style="height:34px;border:1px solid #313844;border-radius:6px;padding:0 8px;background:#0d1014;color:#eef2f6;font:inherit;"><input value="{{ careerOpportunityRank }}" sc-camel-on-change="{{ onCareerOpportunityRank }}" placeholder="Rank" aria-label="Rank filter" style="height:34px;border:1px solid #313844;border-radius:6px;padding:0 8px;background:#0d1014;color:#eef2f6;font:inherit;"><input value="{{ careerOpportunityMos }}" sc-camel-on-change="{{ onCareerOpportunityMos }}" placeholder="MOS" aria-label="MOS filter" style="height:34px;border:1px solid #313844;border-radius:6px;padding:0 8px;background:#0d1014;color:#eef2f6;font:inherit;"><input value="{{ careerOpportunityLocation }}" sc-camel-on-change="{{ onCareerOpportunityLocation }}" placeholder="Location" aria-label="Location filter" style="height:34px;border:1px solid #313844;border-radius:6px;padding:0 8px;background:#0d1014;color:#eef2f6;font:inherit;"><button type="submit" style="height:34px;border:1px solid #313844;border-radius:6px;background:#1a2027;color:#eef2f6;font:inherit;font-weight:600;cursor:pointer;">Apply filters</button><button type="button" sc-camel-on-click="{{ clearCareerOpportunityFilters }}" style="height:34px;border:1px solid #313844;border-radius:6px;background:#1a2027;color:#eef2f6;font:inherit;font-weight:600;cursor:pointer;">Clear filters</button><button type="button" sc-camel-on-click="{{ refreshCareerOpportunities }}" style="height:34px;border:1px solid #b21f2d;border-radius:6px;background:#b21f2d;color:#f5ebe9;font:inherit;font-weight:700;cursor:pointer;">Refresh listings</button></form>\n'
        '        <span aria-live="polite" style="color:#8a94a0;font-size:0.74rem;">{{ careerOpportunityStatus }} {{ careerOpportunitySortFallback }}</span><sc-if value="{{ careerOpportunityRecordsEmpty }}" hint-placeholder-val="{{ false }}"><span style="color:#8a94a0;font-size:0.74rem;">No matching structured listings.</span></sc-if><div style="display:grid;gap:8px;"><sc-for list="{{ careerOpportunityRecords }}" as="item" hint-placeholder-count="2"><article style="padding:10px;border:1px solid #313844;border-radius:6px;background:#0d1014;display:grid;gap:5px;"><div style="display:flex;justify-content:space-between;gap:10px;"><strong style="font-size:0.8rem;">{{ item.title }}</strong><span style="color:#8a94a0;font-size:0.7rem;">{{ item.component }}</span></div><span style="color:#aab4bf;font-size:0.72rem;">{{ item.rank }} · {{ item.mos }} · {{ item.unit }} · {{ item.location }} · {{ item.duration }}</span><span style="color:#8a94a0;font-size:0.7rem;">{{ item.publishedLabel }} · {{ item.dueLabel }}</span><span style="color:#aab4bf;font-size:0.72rem;">{{ item.description }}</span><a href="{{ item.url }}" target="_blank" rel="noopener" style="color:#7db2e0;font-size:0.72rem;font-weight:600;">Open official listing ↗</a></article></sc-for></div>\n'
        "      </section>\n\n"
        '      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">\n',
    ),
    (
        "career opportunities: add visible filters and listing rows",
        "Clear filters",
        '        <p style="margin:0;padding-top:9px;border-top:1px solid #313844;color:#d6bd7a;font-size:0.7rem;line-height:1.45;">Availability and eligibility require verification at the official source. The dashboard does not authenticate, apply, or infer missing dates or requirements.</p>\n'
        "      </section>\n\n"
        '      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">\n',
    ...
    (
        "source library: add local state",
        "    sourceLibrarySources: [],\n",
        '    familyContactEmail: "",\n',
        '    familyContactEmail: "",\n'
        "    sourceLibrarySources: [],\n"
        '    sourceLibraryStatus: "",\n'
        '    sourceLibraryUrl: "",\n'
        '    sourceLibraryTitle: "",\n'
        '    sourceLibraryPublisher: "",\n'
        "    sourceLibraryPreview: null,\n"
        "    sourceLibraryApproved: false,\n"
        "    sourceLibraryRecheck: null,\n"
        "    sourceLibraryRecheckApproved: false,\n"
        '    sourceLibraryQuery: "",\n'
        "    sourceLibraryResults: [],\n",
    ),
    (
        "source library: load saved sources on mount",
        "    this._loadSourceLibrary();",
        "    this._loadFamilyReadiness();\n    this._loadRealNotes();\n",
        "    this._loadFamilyReadiness();\n    this._loadSourceLibrary();\n    this._loadRealNotes();\n",
    ),
    (
        "source library: add approval-bound request helpers",
        "  async _loadSourceLibrary() {",
        "  toggleBenchAdd(idx) {\n",
        "  async _loadSourceLibrary() {\n"
        "    const userKey = this.userKey || this._resolveUserKey();\n"
        '    try { const res = await fetch("/documents/source-library?user_key=" + encodeURIComponent(userKey), { headers: this._apiHeaders() }); if (!res.ok) throw new Error("load failed"); const sources = await res.json(); this.setState({ sourceLibrarySources: sources, sourceLibraryStatus: sources.length ? "" : "No saved public sources yet." }); } catch (_) { this.setState({ sourceLibraryStatus: "Saved sources could not be loaded." }); }\n'
        "  }\n"
        "  async previewSourceLibraryFetch(e) {\n"
        "    e.preventDefault(); const userKey = this.userKey || this._resolveUserKey();\n"
        '    this.setState({ sourceLibraryStatus: "Preparing approval preview…", sourceLibraryPreview: null, sourceLibraryApproved: false });\n'
        '    try { const res = await fetch("/documents/source-library/preview", { method: "POST", headers: this._apiHeaders({ "Content-Type": "application/json" }), body: JSON.stringify({ user_key: userKey, url: this.state.sourceLibraryUrl, title: this.state.sourceLibraryTitle || null, publisher: this.state.sourceLibraryPublisher || null }) }); const data = await res.json(); if (!res.ok) throw new Error(data.detail || "Preview failed"); this.setState({ sourceLibraryPreview: data, sourceLibraryStatus: "Review the exact URL and acknowledge before retrieval." }); } catch (err) { this.setState({ sourceLibraryStatus: String((err && err.message) || err) }); }\n'
        "  }\n"
        "  async fetchSourceLibrary() {\n"
        "    const preview = this.state.sourceLibraryPreview; if (!preview || !this.state.sourceLibraryApproved) return; const userKey = this.userKey || this._resolveUserKey();\n"
        '    this.setState({ sourceLibraryStatus: "Retrieving the approved source…" });\n'
        '    try { const res = await fetch("/documents/source-library/fetch", { method: "POST", headers: this._apiHeaders({ "Content-Type": "application/json" }), body: JSON.stringify({ user_key: userKey, url: this.state.sourceLibraryUrl, title: this.state.sourceLibraryTitle || null, publisher: this.state.sourceLibraryPublisher || null, preview, approval: { approval_digest: preview.approval_digest, acknowledged: true, acknowledged_finding_categories: preview.finding_categories || [] } }) }); const data = await res.json(); if (!res.ok) throw new Error(data.detail || "Retrieval failed"); this.setState({ sourceLibraryPreview: null, sourceLibraryApproved: false, sourceLibraryUrl: "", sourceLibraryTitle: "", sourceLibraryPublisher: "", sourceLibraryStatus: "Saved locally. Review its trust status before use." }); await this._loadSourceLibrary(); } catch (err) { this.setState({ sourceLibraryStatus: String((err && err.message) || err) }); }\n'
        "  }\n"
        "  async removeSourceLibrary(sourceId) {\n"
        "    const userKey = this.userKey || this._resolveUserKey();\n"
        '    try { const res = await fetch("/documents/source-library/" + encodeURIComponent(sourceId) + "?user_key=" + encodeURIComponent(userKey), { method: "DELETE", headers: this._apiHeaders() }); if (!res.ok) throw new Error("Removal failed"); this.setState({ sourceLibraryStatus: "Saved source removed." }); await this._loadSourceLibrary(); } catch (_) { this.setState({ sourceLibraryStatus: "Source could not be removed." }); }\n'
        "  }\n"
        "  async previewSourceLibraryRecheck(sourceId) {\n"
        "    const userKey = this.userKey || this._resolveUserKey();\n"
        '    this.setState({ sourceLibraryStatus: "Preparing recheck preview…", sourceLibraryRecheck: null, sourceLibraryRecheckApproved: false });\n'
        '    try { const res = await fetch("/documents/source-library/" + encodeURIComponent(sourceId) + "/recheck-preview?user_key=" + encodeURIComponent(userKey), { method: "POST", headers: this._apiHeaders() }); const preview = await res.json(); if (!res.ok) throw new Error(preview.detail || "Recheck preview failed"); this.setState({ sourceLibraryRecheck: { sourceId, preview }, sourceLibraryStatus: "Review the recheck preview and acknowledge before retrieval." }); } catch (err) { this.setState({ sourceLibraryStatus: String((err && err.message) || err) }); }\n'
        "  }\n"
        "  async recheckSourceLibrary() {\n"
        "    const pending = this.state.sourceLibraryRecheck; if (!pending || !this.state.sourceLibraryRecheckApproved) return; const userKey = this.userKey || this._resolveUserKey(); const preview = pending.preview;\n"
        '    this.setState({ sourceLibraryStatus: "Rechecking the approved source…" });\n'
        '    try { const res = await fetch("/documents/source-library/" + encodeURIComponent(pending.sourceId) + "/recheck", { method: "POST", headers: this._apiHeaders({ "Content-Type": "application/json" }), body: JSON.stringify({ user_key: userKey, preview, approval: { approval_digest: preview.approval_digest, acknowledged: true, acknowledged_finding_categories: preview.finding_categories || [] } }) }); const data = await res.json(); if (!res.ok) throw new Error(data.detail || "Recheck failed"); this.setState({ sourceLibraryRecheck: null, sourceLibraryRecheckApproved: false, sourceLibraryStatus: data.content_changed ? "Source changed; the local copy was retained and marked for review." : "Recheck complete; no retained-content change detected." }); await this._loadSourceLibrary(); } catch (err) { this.setState({ sourceLibraryStatus: String((err && err.message) || err) }); }\n'
        "  }\n"
        "  async retrieveSourceLibrary(e) {\n"
        "    e.preventDefault(); const query = (this.state.sourceLibraryQuery || \"\").trim(); if (!query) return; const userKey = this.userKey || this._resolveUserKey();\n"
        '    this.setState({ sourceLibraryStatus: "Searching saved local sources…", sourceLibraryResults: [] });\n'
        '    try { const res = await fetch("/documents/source-library/retrieve", { method: "POST", headers: this._apiHeaders({ "Content-Type": "application/json" }), body: JSON.stringify({ user_key: userKey, query, limit: 5 }) }); const data = await res.json(); if (!res.ok) throw new Error(data.detail || "Search failed"); this.setState({ sourceLibraryResults: data.results || [], sourceLibraryStatus: (data.results || []).length ? "" : "No matching active UNCLASSIFIED local sources." }); } catch (err) { this.setState({ sourceLibraryStatus: String((err && err.message) || err) }); }\n'
        "  }\n\n"
        "  toggleBenchAdd(idx) {\n",
    ),
    (
        "source library: compute display bindings",
        "    const sourceLibraryRows = (this.state.sourceLibrarySources || []).map((source) => ({",
        "    const benchCards = this.state.benchCards.map((c, idx) => ({\n",
        "    const sourceLibraryRows = (this.state.sourceLibrarySources || []).map((source) => ({\n"
        '      id: source.source_id, title: source.title || source.original_url, publisher: source.publisher || "Publisher not supplied", meta: (source.media_type || "source").replace("application/", "") + " · " + (source.trust_status || "needs_review").replace(/_/g, " ") + " · " + new Date(source.retrieved_at).toLocaleDateString(), stale: source.trust_status === "watch" || source.lifecycle === "stale", onRecheck: () => this.previewSourceLibraryRecheck(source.source_id), onRemove: () => this.removeSourceLibrary(source.source_id)\n'
        "    }));\n"
        '    const sourceLibraryResults = (this.state.sourceLibraryResults || []).map((result) => ({ excerpt: result.excerpt, title: result.citation && result.citation.title || "Saved source", url: result.citation && result.citation.url || "", trust: result.source_trust && result.source_trust.status || "needs_review", retrieved: result.citation && result.citation.retrieved_at ? new Date(result.citation.retrieved_at).toLocaleString() : "Retrieval date unavailable", hash: result.citation && result.citation.source_hash || "" }));\n'
        "    const sourceLibraryPreview = this.state.sourceLibraryPreview;\n"
        "    const sourceLibraryRecheck = this.state.sourceLibraryRecheck;\n\n"
        "    const benchCards = this.state.benchCards.map((c, idx) => ({\n",
    ),
    (
    ...
    (
        "source library: render Bench Files panel",
        '<h3 style="margin:0 0 4px;font-size:1.02rem;font-weight:700;">Source Library</h3>',
        '      <div style="border:1px dashed #3a4450;border-radius:8px;background:#12161b;padding:16px 18px;">\n        <h2 style="margin:0;font-size:1.16rem;font-weight:700;">Bench / Files</h2>\n        <p style="margin:6px 0 0;color:#aab4bf;font-size:0.88rem;line-height:1.5;max-width:70ch;"><strong style="color:#eef2f6;">Use this page to</strong> keep local references and reusable scaffolds close. <strong style="color:#eef2f6;">You get</strong> your uploaded files, a template library, MOS shortcuts, and curated doctrine notes — the format, not a blank page.</p>\n      </div>\n      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">\n',
        '      <div style="border:1px dashed #3a4450;border-radius:8px;background:#12161b;padding:16px 18px;">\n        <h2 style="margin:0;font-size:1.16rem;font-weight:700;">Bench / Files</h2>\n        <p style="margin:6px 0 0;color:#aab4bf;font-size:0.88rem;line-height:1.5;max-width:70ch;"><strong style="color:#eef2f6;">Use this page to</strong> keep local references and reusable scaffolds close. <strong style="color:#eef2f6;">You get</strong> your uploaded files, a template library, MOS shortcuts, and curated doctrine notes — the format, not a blank page.</p>\n      </div>\n      <section style="border:1px solid #313844;border-radius:8px;background:#12161b;padding:18px;display:grid;gap:12px;"><div><h3 style="margin:0 0 4px;font-size:1.02rem;font-weight:700;">Source Library</h3><p style="margin:0;color:#8a94a0;font-size:0.82rem;line-height:1.45;">Save one approved public HTML page or direct PDF locally for cited planning use. URL entry only prepares a preview; it never retrieves content by itself.</p></div><form sc-camel-on-submit="{{ onPreviewSourceLibraryFetch }}" style="display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:8px;"><input required type="url" value="{{ sourceLibraryUrl }}" sc-camel-on-change="{{ onSourceLibraryUrl }}" placeholder="Exact public URL" aria-label="Exact public source URL" style="height:34px;border:1px solid #313844;border-radius:6px;padding:0 10px;background:#0d1014;color:#eef2f6;font:inherit;font-size:0.8rem;"><input value="{{ sourceLibraryTitle }}" sc-camel-on-change="{{ onSourceLibraryTitle }}" placeholder="Title (optional)" aria-label="Source title" style="height:34px;border:1px solid #313844;border-radius:6px;padding:0 10px;background:#0d1014;color:#eef2f6;font:inherit;font-size:0.8rem;"><input value="{{ sourceLibraryPublisher }}" sc-camel-on-change="{{ onSourceLibraryPublisher }}" placeholder="Publisher (optional)" aria-label="Source publisher" style="height:34px;border:1px solid #313844;border-radius:6px;padding:0 10px;background:#0d1014;color:#eef2f6;font:inherit;font-size:0.8rem;"><button type="submit" style="height:34px;padding:0 12px;border:1px solid #313844;border-radius:6px;background:#1a2027;color:#eef2f6;font:inherit;font-weight:600;font-size:0.78rem;cursor:pointer;">Preview</button></form><sc-if value="{{ sourceLibraryPreview }}" hint-placeholder-val="{{ false }}"><div style="padding:10px;border:1px solid #3a4450;border-radius:6px;background:#0d1014;display:grid;gap:7px;"><strong style="font-size:0.78rem;">Approval required · 1 external retrieval</strong><span style="color:#aab4bf;font-size:0.72rem;word-break:break-all;">{{ sourceLibraryPreview.url }}</span><label style="display:flex;gap:7px;align-items:flex-start;color:#c7cfd8;font-size:0.72rem;"><input type="checkbox" checked="{{ sourceLibraryApproved }}" sc-camel-on-change="{{ onSourceLibraryApproval }}"><span>I reviewed the URL and warnings. Retrieve and save this exact source locally.</span></label><button type="button" disabled="{{ !sourceLibraryApproved }}" sc-camel-on-click="{{ fetchSourceLibrary }}" style="height:32px;padding:0 12px;border:1px solid #b21f2d;border-radius:6px;background:#b21f2d;color:#f5ebe9;font:inherit;font-weight:600;font-size:0.76rem;cursor:pointer;">Retrieve approved source</button></div></sc-if><sc-if value="{{ sourceLibraryRecheck }}" hint-placeholder-val="{{ false }}"><div style="padding:10px;border:1px solid #d6bd7a;border-radius:6px;background:#17150e;display:grid;gap:7px;"><strong style="font-size:0.78rem;">Recheck approval required · 1 external retrieval</strong><span style="color:#d6bd7a;font-size:0.72rem;word-break:break-all;">{{ sourceLibraryRecheck.preview.url }}</span><label style="display:flex;gap:7px;align-items:flex-start;color:#c7cfd8;font-size:0.72rem;"><input type="checkbox" checked="{{ sourceLibraryRecheckApproved }}" sc-camel-on-change="{{ onSourceLibraryRecheckApproval }}"><span>I reviewed the exact recheck URL and authorize this retrieval.</span></label><button type="button" disabled="{{ !sourceLibraryRecheckApproved }}" sc-camel-on-click="{{ recheckSourceLibrary }}" style="height:32px;padding:0 12px;border:1px solid #d6bd7a;border-radius:6px;background:#2c2610;color:#f5ebe9;font:inherit;font-weight:600;font-size:0.76rem;cursor:pointer;">Recheck approved source</button></div></sc-if><span aria-live="polite" style="color:#8a94a0;font-size:0.74rem;">{{ sourceLibraryStatus }}</span><div style="display:grid;gap:6px;"><sc-for list="{{ sourceLibraryRows }}" as="source" hint-placeholder-count="2"><div style="padding:9px;border:1px solid #313844;border-radius:6px;background:#0d1014;display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:center;"><span><strong style="display:block;font-size:0.76rem;">{{ source.title }}</strong><small style="color:#8a94a0;font-size:0.68rem;">{{ source.publisher }} · {{ source.meta }}</small></span><button type="button" sc-camel-on-click="{{ source.onRecheck }}" style="height:28px;border:1px solid #313844;border-radius:5px;background:#1a2027;color:#eef2f6;font:inherit;font-size:0.7rem;cursor:pointer;">Recheck</button><button type="button" sc-camel-on-click="{{ source.onRemove }}" style="height:28px;border:1px solid #3a2226;border-radius:5px;background:#1b1114;color:#d8a0a5;font:inherit;font-size:0.7rem;cursor:pointer;">Remove</button></div></sc-for></div><form sc-camel-on-submit="{{ retrieveSourceLibrary }}" style="display:flex;gap:8px;padding-top:4px;border-top:1px solid #313844;"><input required value="{{ sourceLibraryQuery }}" sc-camel-on-change="{{ onSourceLibraryQuery }}" placeholder="Search saved local sources" aria-label="Search saved local sources" style="flex:1;height:34px;border:1px solid #313844;border-radius:6px;padding:0 10px;background:#0d1014;color:#eef2f6;font:inherit;font-size:0.8rem;"><button type="submit" style="height:34px;padding:0 12px;border:1px solid #313844;border-radius:6px;background:#1a2027;color:#eef2f6;font:inherit;font-weight:600;font-size:0.78rem;cursor:pointer;">Search</button></form><div style="display:grid;gap:7px;"><sc-for list="{{ sourceLibraryResults }}" as="result" hint-placeholder-count="2"><article style="padding:9px;border-left:2px solid #d6bd7a;background:#0d1014;"><strong style="display:block;font-size:0.76rem;">{{ result.title }}</strong><p style="margin:5px 0;color:#aab4bf;font-size:0.72rem;line-height:1.4;">{{ result.excerpt }}</p><small style="color:#8a94a0;font-size:0.66rem;">{{ result.trust }} · retrieved {{ result.retrieved }} · {{ result.hash }}</small><a href="{{ result.url }}" target="_blank" rel="noopener" style="display:block;margin-top:4px;color:#7db2e0;font-size:0.68rem;">Open original source ↗</a></article></sc-for></div><p style="margin:0;color:#d6bd7a;font-size:0.7rem;line-height:1.4;">DRAFT — Verify all references against current official sources before acting.</p></section>\n      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">\n',
    ),
    (
        "civil network: add local state",
        "    civilNetworkStatus: \"\",",
        "    sourceLibrarySources: [],\n",
        "    sourceLibrarySources: [],\n"
        "    civilNetworks: [],\n"
        "    civilNetworkActive: null,\n"
        "    civilNetworkStatus: \"\",\n"
        "    civilNetworkTitle: \"\",\n"
        "    civilNetworkEventId: \"\",\n"
        "    civilNetworkPurpose: \"\",\n"
        "    civilNetworkNodeName: \"\",\n"
        "    civilNetworkNodeKind: \"organization\",\n"
        "    civilNetworkNodeEvidenceKind: \"planning_hypothesis\",\n"
        "    civilNetworkNodeEvidenceTitle: \"\",\n"
        "    civilNetworkNodeEvidenceUrl: \"\",\n"
        "    civilNetworkNodeEvidenceExcerpt: \"\",\n"
        "    civilNetworkNodeConfidence: \"low\",\n"
        "    civilNetworkNodeReview: \"needs_review\",\n"
        "    civilNetworkEdgeFrom: \"\",\n"
        "    civilNetworkEdgeTo: \"\",\n"
        "    civilNetworkEdgeKind: \"coordination\",\n"
        "    civilNetworkEdgeEvidenceKind: \"planning_hypothesis\",\n"
        "    civilNetworkEdgeDescription: \"\",\n"
        "    civilNetworkEdgeConfidence: \"low\",\n"
        "    civilNetworkEdgeReview: \"needs_review\",\n"
        "    civilNetworkFilterKind: \"\",\n"
        "    civilNetworkFilterEvidence: \"\",\n"
        "    civilNetworkFilterConfidence: \"\",\n"
        "    civilNetworkFilterReview: \"\",\n"
        "    civilNetworkWarningsOnly: false,\n",
    ),
    (
        "civil network: load event workspace on mount",
        "    this._loadCivilNetworks();",
        "    this._loadSourceLibrary();\n",
        "    this._loadSourceLibrary();\n    this._loadCivilNetworks();\n",
    ),
    (
        "civil network: add API helpers",
        "  async _loadCivilNetworks() {",
        "  async _loadSourceLibrary() {\n",
        "  async _loadCivilNetworks() {\n"
        "    const userKey = this.userKey || this._resolveUserKey();\n"
        "    try { const res = await fetch(\"/civil-networks?user_key=\" + encodeURIComponent(userKey), { headers: this._apiHeaders() }); if (!res.ok) throw new Error(\"load failed\"); const civilNetworks = await res.json(); const activeId = this.state.civilNetworkActive && this.state.civilNetworkActive.id; const civilNetworkActive = civilNetworks.find((network) => network.id === activeId) || civilNetworks[0] || null; this.setState({ civilNetworks, civilNetworkActive, civilNetworkStatus: civilNetworks.length ? \"\" : \"Create an event-scoped network to begin.\" }); } catch (_) { this.setState({ civilNetworkStatus: \"Civil networks could not be loaded.\" }); }\n"
        "  }\n"
        "  async _saveCivilNetwork(network, created) {\n"
        "    const userKey = this.userKey || this._resolveUserKey(); const url = created ? \"/civil-networks\" : \"/civil-networks/\" + encodeURIComponent(network.id);\n"
        "    const res = await fetch(url, { method: created ? \"POST\" : \"PUT\", headers: this._apiHeaders({ \"Content-Type\": \"application/json\" }), body: JSON.stringify({ user_key: userKey, network }) }); const data = await res.json(); if (!res.ok) throw new Error(data.detail || \"Save failed\"); this.setState({ civilNetworkActive: data }); await this._loadCivilNetworks(); return data;\n"
        "  }\n"
        "  async createCivilNetwork(e) {\n"
        "    e.preventDefault(); const title = (this.state.civilNetworkTitle || \"\").trim(); const eventId = (this.state.civilNetworkEventId || \"\").trim(); const purpose = (this.state.civilNetworkPurpose || \"\").trim(); if (!title || !eventId || !purpose) return; this.setState({ civilNetworkStatus: \"Saving event network…\" }); try { await this._saveCivilNetwork({ title, event_id: eventId, purpose, nodes: [], relationships: [] }, true); this.setState({ civilNetworkTitle: \"\", civilNetworkEventId: \"\", civilNetworkPurpose: \"\", civilNetworkStatus: \"Event network saved.\" }); } catch (err) { this.setState({ civilNetworkStatus: String((err && err.message) || err) }); }\n"
        "  }\n"
        "  selectCivilNetwork(id) { const network = (this.state.civilNetworks || []).find((item) => item.id === id) || null; this.setState({ civilNetworkActive: network, civilNetworkStatus: network ? \"\" : this.state.civilNetworkStatus }); }\n"
        "  async addCivilNetworkNode(e) {\n"
        "    e.preventDefault(); const network = this.state.civilNetworkActive; const name = (this.state.civilNetworkNodeName || \"\").trim(); if (!network || !name) return; const evidenceKind = this.state.civilNetworkNodeEvidenceKind; const title = (this.state.civilNetworkNodeEvidenceTitle || \"\").trim(); const excerpt = (this.state.civilNetworkNodeEvidenceExcerpt || \"\").trim(); const evidence = title && excerpt ? [{ title, url: (this.state.civilNetworkNodeEvidenceUrl || \"\").trim() || null, bibliographic_note: (this.state.civilNetworkNodeEvidenceUrl || \"\").trim() ? null : \"Manual dashboard citation\", retrieved_at: new Date().toISOString().slice(0, 10), trust_status: \"needs_review\", excerpt, confidence: this.state.civilNetworkNodeConfidence, review_state: this.state.civilNetworkNodeReview }] : []; if (evidenceKind === \"sourced_observation\" && !evidence.length) { this.setState({ civilNetworkStatus: \"Sourced observations require a title and excerpt.\" }); return; } const node = { id: \"node-\" + Date.now().toString(36), kind: this.state.civilNetworkNodeKind, display_name: name, evidence_kind: evidenceKind, evidence, confidence: this.state.civilNetworkNodeConfidence, review_state: this.state.civilNetworkNodeReview }; try { await this._saveCivilNetwork(Object.assign({}, network, { nodes: (network.nodes || []).concat([node]) }), false); this.setState({ civilNetworkNodeName: \"\", civilNetworkNodeEvidenceTitle: \"\", civilNetworkNodeEvidenceUrl: \"\", civilNetworkNodeEvidenceExcerpt: \"\", civilNetworkStatus: \"Node saved.\" }); } catch (err) { this.setState({ civilNetworkStatus: String((err && err.message) || err) }); }\n"
        "  }\n"
        "  async addCivilNetworkRelationship(e) {\n"
        "    e.preventDefault(); const network = this.state.civilNetworkActive; if (!network || !this.state.civilNetworkEdgeFrom || !this.state.civilNetworkEdgeTo || !this.state.civilNetworkEdgeDescription) return; const edge = { id: \"edge-\" + Date.now().toString(36), from_node_id: this.state.civilNetworkEdgeFrom, to_node_id: this.state.civilNetworkEdgeTo, kind: this.state.civilNetworkEdgeKind, evidence_kind: this.state.civilNetworkEdgeEvidenceKind, description: this.state.civilNetworkEdgeDescription, evidence: [], confidence: this.state.civilNetworkEdgeConfidence, review_state: this.state.civilNetworkEdgeReview }; try { await this._saveCivilNetwork(Object.assign({}, network, { relationships: (network.relationships || []).concat([edge]) }), false); this.setState({ civilNetworkEdgeDescription: \"\", civilNetworkStatus: \"Relationship saved.\" }); } catch (err) { this.setState({ civilNetworkStatus: String((err && err.message) || err) }); }\n"
        "  }\n"
        "  async snapshotCivilNetwork() {\n"
        "    const network = this.state.civilNetworkActive; if (!network || !window.confirm(\"Create an immutable read-only planning snapshot of this event network?\")) return; const label = window.prompt(\"Snapshot label\", \"Planning snapshot\"); if (!label) return; const userKey = this.userKey || this._resolveUserKey(); try { const res = await fetch(\"/civil-networks/\" + encodeURIComponent(network.id) + \"/snapshots\", { method: \"POST\", headers: this._apiHeaders({ \"Content-Type\": \"application/json\" }), body: JSON.stringify({ user_key: userKey, label }) }); const data = await res.json(); if (!res.ok) throw new Error(data.detail || \"Snapshot failed\"); this.setState({ civilNetworkStatus: \"Snapshot saved: \" + data.label }); await this._loadCivilNetworks(); } catch (err) { this.setState({ civilNetworkStatus: String((err && err.message) || err) }); }\n"
        "  }\n"
        "  civilNetworkMap(network) { return !network ? [] : (network.relationships || []).map((edge) => ({ from: edge.from_node_id, to: edge.to_node_id, label: edge.kind + \" · \" + edge.evidence_kind + \" · \" + edge.confidence, warning: (edge.evidence || []).some((item) => item.trust_status !== \"current\") })); }\n\n"
        "  async _loadSourceLibrary() {\n",
    ),
    (
        "civil network: compute workspace bindings",
        "    const civilNetworkMap = this.civilNetworkMap(civilNetworkActive);",
        "    const sourceLibraryRows = (this.state.sourceLibrarySources || []).map((source) => ({",
        "    const civilNetworkActive = this.state.civilNetworkActive;\n"
        "    const civilNetworkMap = this.civilNetworkMap(civilNetworkActive);\n"
        "    const civilNetworkFilters = { kind: this.state.civilNetworkFilterKind, evidence: this.state.civilNetworkFilterEvidence, confidence: this.state.civilNetworkFilterConfidence, review: this.state.civilNetworkFilterReview, warnings: this.state.civilNetworkWarningsOnly };\n"
        "    const civilNetworkEdges = civilNetworkMap.filter((edge) => (!civilNetworkFilters.kind || edge.label.indexOf(civilNetworkFilters.kind) >= 0) && (!civilNetworkFilters.evidence || edge.label.indexOf(civilNetworkFilters.evidence) >= 0) && (!civilNetworkFilters.confidence || edge.label.indexOf(civilNetworkFilters.confidence) >= 0) && (!civilNetworkFilters.warnings || edge.warning));\n"
        "    const sourceLibraryRows = (this.state.sourceLibrarySources || []).map((source) => ({",
    ),
    (
        "civil network: expose workspace bindings",
        "      civilNetworkMap, civilNetworkEdges,",
        "      benchCards, benchCardsGrid, projectFilesCard, workflows,\n"
        "      sourceLibraryRows, sourceLibraryResults, sourceLibraryPreview, sourceLibraryRecheck, sourceLibraryStatus: this.state.sourceLibraryStatus,",
        "      civilNetworkMap, civilNetworkEdges, civilNetworkActive, civilNetworks: (this.state.civilNetworks || []).map((network) => ({ id: network.id, label: network.title + \" · \" + network.event_id })), civilNetworkStatus: this.state.civilNetworkStatus, civilNetworkTitle: this.state.civilNetworkTitle, civilNetworkEventId: this.state.civilNetworkEventId, civilNetworkPurpose: this.state.civilNetworkPurpose, civilNetworkNodeName: this.state.civilNetworkNodeName, civilNetworkNodeKind: this.state.civilNetworkNodeKind, civilNetworkNodeEvidenceKind: this.state.civilNetworkNodeEvidenceKind, civilNetworkNodeEvidenceTitle: this.state.civilNetworkNodeEvidenceTitle, civilNetworkNodeEvidenceUrl: this.state.civilNetworkNodeEvidenceUrl, civilNetworkNodeEvidenceExcerpt: this.state.civilNetworkNodeEvidenceExcerpt, civilNetworkNodeConfidence: this.state.civilNetworkNodeConfidence, civilNetworkNodeReview: this.state.civilNetworkNodeReview, civilNetworkEdgeFrom: this.state.civilNetworkEdgeFrom, civilNetworkEdgeTo: this.state.civilNetworkEdgeTo, civilNetworkEdgeKind: this.state.civilNetworkEdgeKind, civilNetworkEdgeEvidenceKind: this.state.civilNetworkEdgeEvidenceKind, civilNetworkEdgeDescription: this.state.civilNetworkEdgeDescription, civilNetworkEdgeConfidence: this.state.civilNetworkEdgeConfidence, civilNetworkEdgeReview: this.state.civilNetworkEdgeReview, civilNetworkFilterKind: this.state.civilNetworkFilterKind, civilNetworkFilterEvidence: this.state.civilNetworkFilterEvidence, civilNetworkFilterConfidence: this.state.civilNetworkFilterConfidence, civilNetworkFilterReview: this.state.civilNetworkFilterReview, civilNetworkWarningsOnly: this.state.civilNetworkWarningsOnly, onCivilNetworkSelect: (e) => this.selectCivilNetwork(e.target.value), onCreateCivilNetwork: (e) => this.createCivilNetwork(e), onAddCivilNetworkNode: (e) => this.addCivilNetworkNode(e), onAddCivilNetworkRelationship: (e) => this.addCivilNetworkRelationship(e), snapshotCivilNetwork: () => this.snapshotCivilNetwork(), onCivilNetworkTitle: (e) => this.setState({ civilNetworkTitle: e.target.value }), onCivilNetworkEventId: (e) => this.setState({ civilNetworkEventId: e.target.value }), onCivilNetworkPurpose: (e) => this.setState({ civilNetworkPurpose: e.target.value }), onCivilNetworkNodeName: (e) => this.setState({ civilNetworkNodeName: e.target.value }), onCivilNetworkNodeKind: (e) => this.setState({ civilNetworkNodeKind: e.target.value }), onCivilNetworkNodeEvidenceKind: (e) => this.setState({ civilNetworkNodeEvidenceKind: e.target.value }), onCivilNetworkNodeEvidenceTitle: (e) => this.setState({ civilNetworkNodeEvidenceTitle: e.target.value }), onCivilNetworkNodeEvidenceUrl: (e) => this.setState({ civilNetworkNodeEvidenceUrl: e.target.value }), onCivilNetworkNodeEvidenceExcerpt: (e) => this.setState({ civilNetworkNodeEvidenceExcerpt: e.target.value }), onCivilNetworkNodeConfidence: (e) => this.setState({ civilNetworkNodeConfidence: e.target.value }), onCivilNetworkNodeReview: (e) => this.setState({ civilNetworkNodeReview: e.target.value }), onCivilNetworkEdgeFrom: (e) => this.setState({ civilNetworkEdgeFrom: e.target.value }), onCivilNetworkEdgeTo: (e) => this.setState({ civilNetworkEdgeTo: e.target.value }), onCivilNetworkEdgeKind: (e) => this.setState({ civilNetworkEdgeKind: e.target.value }), onCivilNetworkEdgeEvidenceKind: (e) => this.setState({ civilNetworkEdgeEvidenceKind: e.target.value }), onCivilNetworkEdgeDescription: (e) => this.setState({ civilNetworkEdgeDescription: e.target.value }), onCivilNetworkEdgeConfidence: (e) => this.setState({ civilNetworkEdgeConfidence: e.target.value }), onCivilNetworkEdgeReview: (e) => this.setState({ civilNetworkEdgeReview: e.target.value }), onCivilNetworkFilterKind: (e) => this.setState({ civilNetworkFilterKind: e.target.value }), onCivilNetworkFilterEvidence: (e) => this.setState({ civilNetworkFilterEvidence: e.target.value }), onCivilNetworkFilterConfidence: (e) => this.setState({ civilNetworkFilterConfidence: e.target.value }), onCivilNetworkFilterReview: (e) => this.setState({ civilNetworkFilterReview: e.target.value }), onCivilNetworkWarningsOnly: (e) => this.setState({ civilNetworkWarningsOnly: e.target.checked }),\n"
        "      benchCards, benchCardsGrid, projectFilesCard, workflows,\n"
        "      civilNetworkMap, civilNetworkEdges, civilNetworkActive, civilNetworks: (this.state.civilNetworks || []).map((network) => ({ id: network.id, label: network.title + \" · \" + network.event_id })), civilNetworkStatus: this.state.civilNetworkStatus, civilNetworkTitle: this.state.civilNetworkTitle, civilNetworkEventId: this.state.civilNetworkEventId, civilNetworkPurpose: this.state.civilNetworkPurpose, civilNetworkNodeName: this.state.civilNetworkNodeName, civilNetworkNodeKind: this.state.civilNetworkNodeKind, civilNetworkNodeEvidenceKind: this.state.civilNetworkNodeEvidenceKind, civilNetworkNodeEvidenceTitle: this.state.civilNetworkNodeEvidenceTitle, civilNetworkNodeEvidenceUrl: this.state.civilNetworkNodeEvidenceUrl, civilNetworkNodeEvidenceExcerpt: this.state.civilNetworkNodeEvidenceExcerpt, civilNetworkNodeConfidence: this.state.civilNetworkNodeConfidence, civilNetworkNodeReview: this.state.civilNetworkNodeReview, civilNetworkEdgeFrom: this.state.civilNetworkEdgeFrom, civilNetworkEdgeTo: this.state.civilNetworkEdgeTo, civilNetworkEdgeKind: this.state.civilNetworkEdgeKind, civilNetworkEdgeEvidenceKind: this.state.civilNetworkEdgeEvidenceKind, civilNetworkEdgeDescription: this.state.civilNetworkEdgeDescription, civilNetworkEdgeConfidence: this.state.civilNetworkEdgeConfidence, civilNetworkEdgeReview: this.state.civilNetworkEdgeReview, civilNetworkFilterKind: this.state.civilNetworkFilterKind, civilNetworkFilterEvidence: this.state.civilNetworkFilterEvidence, civilNetworkFilterConfidence: this.state.civilNetworkFilterConfidence, civilNetworkFilterReview: this.state.civilNetworkFilterReview, civilNetworkWarningsOnly: this.state.civilNetworkWarningsOnly, onCivilNetworkSelect: (e) => this.selectCivilNetwork(e.target.value), onCreateCivilNetwork: (e) => this.createCivilNetwork(e), onAddCivilNetworkNode: (e) => this.addCivilNetworkNode(e), onAddCivilNetworkRelationship: (e) => this.addCivilNetworkRelationship(e), snapshotCivilNetwork: () => this.snapshotCivilNetwork(), onCivilNetworkTitle: (e) => this.setState({ civilNetworkTitle: e.target.value }), onCivilNetworkEventId: (e) => this.setState({ civilNetworkEventId: e.target.value }), onCivilNetworkPurpose: (e) => this.setState({ civilNetworkPurpose: e.target.value }), onCivilNetworkNodeName: (e) => this.setState({ civilNetworkNodeName: e.target.value }), onCivilNetworkNodeKind: (e) => this.setState({ civilNetworkNodeKind: e.target.value }), onCivilNetworkNodeEvidenceKind: (e) => this.setState({ civilNetworkNodeEvidenceKind: e.target.value }), onCivilNetworkNodeEvidenceTitle: (e) => this.setState({ civilNetworkNodeEvidenceTitle: e.target.value }), onCivilNetworkNodeEvidenceUrl: (e) => this.setState({ civilNetworkNodeEvidenceUrl: e.target.value }), onCivilNetworkNodeEvidenceExcerpt: (e) => this.setState({ civilNetworkNodeEvidenceExcerpt: e.target.value }), onCivilNetworkNodeConfidence: (e) => this.setState({ civilNetworkNodeConfidence: e.target.value }), onCivilNetworkNodeReview: (e) => this.setState({ civilNetworkNodeReview: e.target.value }), onCivilNetworkEdgeFrom: (e) => this.setState({ civilNetworkEdgeFrom: e.target.value }), onCivilNetworkEdgeTo: (e) => this.setState({ civilNetworkEdgeTo: e.target.value }), onCivilNetworkEdgeKind: (e) => this.setState({ civilNetworkEdgeKind: e.target.value }), onCivilNetworkEdgeEvidenceKind: (e) => this.setState({ civilNetworkEdgeEvidenceKind: e.target.value }), onCivilNetworkEdgeDescription: (e) => this.setState({ civilNetworkEdgeDescription: e.target.value }), onCivilNetworkEdgeConfidence: (e) => this.setState({ civilNetworkEdgeConfidence: e.target.value }), onCivilNetworkEdgeReview: (e) => this.setState({ civilNetworkEdgeReview: e.target.value }), onCivilNetworkFilterKind: (e) => this.setState({ civilNetworkFilterKind: e.target.value }), onCivilNetworkFilterEvidence: (e) => this.setState({ civilNetworkFilterEvidence: e.target.value }), onCivilNetworkFilterConfidence: (e) => this.setState({ civilNetworkFilterConfidence: e.target.value }), onCivilNetworkFilterReview: (e) => this.setState({ civilNetworkFilterReview: e.target.value }), onCivilNetworkWarningsOnly: (e) => this.setState({ civilNetworkWarningsOnly: e.target.checked }),\n"
        "      sourceLibraryRows, sourceLibraryResults, sourceLibraryPreview, sourceLibraryRecheck, sourceLibraryStatus:",
    ),
    (
        "civil network: render Bench Files workspace",
        'id="civil-network-workspace"',
        '      <section id="civil-network-workspace" style="border:1px solid #47617a;border-radius:8px;background:#10161d;padding:18px;display:grid;gap:12px;"><div><h3 style="margin:0 0 4px;font-size:1.02rem;font-weight:700;">Civil Network / Nodal Analysis</h3><p style="margin:0;color:#aab4bf;font-size:0.82rem;line-height:1.45;">Event-scoped, public-source or cited planning data. This is a derived map—not people tracking, targeting, authority determination, or vulnerability scoring.</p></div><form sc-camel-on-submit="{{ onCreateCivilNetwork }}" style="display:grid;grid-template-columns:1fr 1fr 2fr auto;gap:8px;"><input required value="{{ civilNetworkTitle }}" sc-camel-on-change="{{ onCivilNetworkTitle }}" placeholder="Network title" aria-label="Civil network title"><input required value="{{ civilNetworkEventId }}" sc-camel-on-change="{{ onCivilNetworkEventId }}" placeholder="Event ID" aria-label="Event ID"><input required value="{{ civilNetworkPurpose }}" sc-camel-on-change="{{ onCivilNetworkPurpose }}" placeholder="Purpose" aria-label="Civil network purpose"><button type="submit">Create event network</button></form><select sc-camel-on-change="{{ onCivilNetworkSelect }}" aria-label="Select civil network"><option value="">Select an event network</option><sc-for list="{{ civilNetworks }}" as="network" hint-placeholder-count="2"><option value="{{ network.id }}">{{ network.label }}</option></sc-for></select><sc-if value="{{ civilNetworkActive }}" hint-placeholder-val="{{ false }}"><div style="display:grid;gap:10px;border-top:1px solid #313844;padding-top:10px;"><form sc-camel-on-submit="{{ onAddCivilNetworkNode }}" style="display:grid;grid-template-columns:1fr repeat(4,auto);gap:8px;"><input required value="{{ civilNetworkNodeName }}" sc-camel-on-change="{{ onCivilNetworkNodeName }}" placeholder="Public organization, service, forum, group, or role-holder"><select value="{{ civilNetworkNodeKind }}" sc-camel-on-change="{{ onCivilNetworkNodeKind }}"><option value="organization">Organization</option><option value="service">Service</option><option value="forum">Forum</option><option value="broad_group">Broad group</option></select><select value="{{ civilNetworkNodeEvidenceKind }}" sc-camel-on-change="{{ onCivilNetworkNodeEvidenceKind }}"><option value="planning_hypothesis">Planning hypothesis</option><option value="analytic_inference">Analytic inference</option><option value="sourced_observation">Sourced observation</option></select><select value="{{ civilNetworkNodeConfidence }}" sc-camel-on-change="{{ onCivilNetworkNodeConfidence }}"><option value="low">Low confidence</option><option value="medium">Medium confidence</option><option value="high">High confidence</option></select><select value="{{ civilNetworkNodeReview }}" sc-camel-on-change="{{ onCivilNetworkNodeReview }}"><option value="needs_review">Needs review</option><option value="reviewed">Reviewed</option></select><button type="submit">Add node</button></form><div style="display:grid;grid-template-columns:1fr 1fr 2fr;gap:8px;"><input value="{{ civilNetworkNodeEvidenceTitle }}" sc-camel-on-change="{{ onCivilNetworkNodeEvidenceTitle }}" placeholder="Evidence title / citation"><input value="{{ civilNetworkNodeEvidenceUrl }}" sc-camel-on-change="{{ onCivilNetworkNodeEvidenceUrl }}" placeholder="Public URL (optional)"><input value="{{ civilNetworkNodeEvidenceExcerpt }}" sc-camel-on-change="{{ onCivilNetworkNodeEvidenceExcerpt }}" placeholder="Evidence excerpt or rationale"></div><form sc-camel-on-submit="{{ onAddCivilNetworkRelationship }}" style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 2fr auto;gap:8px;"><select required value="{{ civilNetworkEdgeFrom }}" sc-camel-on-change="{{ onCivilNetworkEdgeFrom }}"><option value="">From node</option><sc-for list="{{ civilNetworkActive.nodes }}" as="node" hint-placeholder-count="2"><option value="{{ node.id }}">{{ node.display_name }}</option></sc-for></select><select required value="{{ civilNetworkEdgeTo }}" sc-camel-on-change="{{ onCivilNetworkEdgeTo }}"><option value="">To node</option><sc-for list="{{ civilNetworkActive.nodes }}" as="node" hint-placeholder-count="2"><option value="{{ node.id }}">{{ node.display_name }}</option></sc-for></select><select value="{{ civilNetworkEdgeKind }}" sc-camel-on-change="{{ onCivilNetworkEdgeKind }}"><option value="coordination">Coordination</option><option value="dependency">Dependency</option><option value="influence">Influence</option><option value="information_flow">Information flow</option><option value="authority_approval">Authority / approval</option><option value="resource_support">Resource support</option><option value="legitimacy_trust">Legitimacy / trust</option></select><select value="{{ civilNetworkEdgeEvidenceKind }}" sc-camel-on-change="{{ onCivilNetworkEdgeEvidenceKind }}"><option value="planning_hypothesis">Planning hypothesis</option><option value="analytic_inference">Analytic inference</option></select><input required value="{{ civilNetworkEdgeDescription }}" sc-camel-on-change="{{ onCivilNetworkEdgeDescription }}" placeholder="Relationship description"><button type="submit">Add relationship</button></form><button type="button" sc-camel-on-click="{{ snapshotCivilNetwork }}">Create immutable snapshot</button><div style="display:flex;gap:6px;flex-wrap:wrap;"><select value="{{ civilNetworkFilterKind }}" sc-camel-on-change="{{ onCivilNetworkFilterKind }}"><option value="">All relationships</option><option value="coordination">Coordination</option><option value="dependency">Dependency</option><option value="influence">Influence</option></select><select value="{{ civilNetworkFilterEvidence }}" sc-camel-on-change="{{ onCivilNetworkFilterEvidence }}"><option value="">All evidence</option><option value="sourced_observation">Sourced observation</option><option value="analytic_inference">Analytic inference</option><option value="planning_hypothesis">Planning hypothesis</option></select><select value="{{ civilNetworkFilterConfidence }}" sc-camel-on-change="{{ onCivilNetworkFilterConfidence }}"><option value="">All confidence</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select><select value="{{ civilNetworkFilterReview }}" sc-camel-on-change="{{ onCivilNetworkFilterReview }}"><option value="">All review states</option><option value="reviewed">Reviewed</option><option value="needs_review">Needs review</option></select><label><input type="checkbox" checked="{{ civilNetworkWarningsOnly }}" sc-camel-on-change="{{ onCivilNetworkWarningsOnly }}"> Warnings only</label></div><div style="display:grid;gap:6px;"><sc-for list="{{ civilNetworkEdges }}" as="edge" hint-placeholder-count="2"><div style="padding:8px;border-left:3px solid #47617a;background:#0d1014;"><strong>{{ edge.from }} → {{ edge.to }}</strong><span style="margin-left:8px;color:#aab4bf;font-size:0.75rem;">{{ edge.label }}</span><sc-if value="{{ edge.warning }}" hint-placeholder-val="{{ false }}"><span style="margin-left:8px;color:#d6bd7a;font-size:0.72rem;">noncurrent evidence warning</span></sc-if></div></sc-for></div></div></sc-if><span aria-live="polite" style="color:#8a94a0;font-size:0.74rem;">{{ civilNetworkStatus }}</span><p style="margin:0;color:#d6bd7a;font-size:0.7rem;">DRAFT — Verify all references against current official sources before acting.</p></section>\n      <section style="border:1px solid #313844;border-radius:8px;background:#12161b;padding:18px;display:grid;gap:12px;"><div><h3 style="margin:0 0 4px;font-size:1.02rem;font-weight:700;">Source Library</h3>',
        '      <section style="border:1px solid #313844;border-radius:8px;background:#12161b;padding:18px;display:grid;gap:12px;"><div><h3 style="margin:0 0 4px;font-size:1.02rem;font-weight:700;">Source Library</h3>',
    ),
]

# Civil-network recovery: the initial Task 4 addition was left with its final
# find/replacement reversed, so its helpers were never written into the bundle.
# Keep this correction close to the patch declaration: the Source Library
# opening tag is the stable insertion point in the decoded design-tool export.
_civil_workspace_entry = PATCHES[-1]
_civil_workspace = _civil_workspace_entry[2]
_civil_workspace = _civil_workspace.replace(
    '<option value="broad_group">Broad group</option>',
    '<option value="broad_group">Broad group</option><option value="public_role_holder">Public role-holder</option>',
).replace(
    '<option value="planning_hypothesis">Planning hypothesis</option><option value="analytic_inference">Analytic inference</option></select><input required value="{{ civilNetworkEdgeDescription }}"',
    '<option value="planning_hypothesis">Planning hypothesis</option><option value="analytic_inference">Analytic inference</option><option value="sourced_observation">Sourced observation</option></select><input required value="{{ civilNetworkEdgeDescription }}"',
).replace(
    '<option value="influence">Influence</option></select><select value="{{ civilNetworkFilterEvidence }}"',
    '<option value="influence">Influence</option><option value="information_flow">Information flow</option><option value="authority_approval">Authority / approval</option><option value="resource_support">Resource support</option><option value="legitimacy_trust">Legitimacy / trust</option></select><select value="{{ civilNetworkFilterEvidence }}"',
)
PATCHES[-1] = (
    _civil_workspace_entry[0],
    _civil_workspace_entry[1],
    _civil_workspace_entry[3],
    _civil_workspace + _civil_workspace_entry[3],
)


def _replace_patch_new(label: str, old: str, new: str) -> None:
    """Amend one decoded-bundle patch without duplicating its large template."""
    for index, entry in enumerate(PATCHES):
        if entry[0] != label:
            continue
        updated = entry[-1].replace(old, new, 1)
        if updated == entry[-1]:
            raise RuntimeError(f"Civil-network recovery target missing from patch: {label}")
        PATCHES[index] = (*entry[:-1], updated)
        return
    raise RuntimeError(f"Civil-network recovery patch missing: {label}")


_replace_patch_new(
    "civil network: add API helpers",
    'const node = { id: "node-" + Date.now().toString(36), kind: this.state.civilNetworkNodeKind, display_name: name, evidence_kind: evidenceKind, evidence, confidence: this.state.civilNetworkNodeConfidence, review_state: this.state.civilNetworkNodeReview };',
    'let publicRole = null; let organization = null; let eventRelevance = null; if (this.state.civilNetworkNodeKind === "public_role_holder") { publicRole = window.prompt("Public role (required)", "") || ""; organization = window.prompt("Public organization (required)", "") || ""; eventRelevance = window.prompt("Why is this public role relevant to this event?", "") || ""; if (!publicRole.trim() || !organization.trim() || !eventRelevance.trim() || !evidence.length) { this.setState({ civilNetworkStatus: "Public role-holders require public role, organization, event relevance, and cited evidence." }); return; } } const node = { id: "node-" + Date.now().toString(36), kind: this.state.civilNetworkNodeKind, display_name: name, public_role: publicRole, organization, event_relevance: eventRelevance, evidence_kind: evidenceKind, evidence, confidence: this.state.civilNetworkNodeConfidence, review_state: this.state.civilNetworkNodeReview };',
)
_replace_patch_new(
    "civil network: add API helpers",
    'const edge = { id: "edge-" + Date.now().toString(36), from_node_id: this.state.civilNetworkEdgeFrom, to_node_id: this.state.civilNetworkEdgeTo, kind: this.state.civilNetworkEdgeKind, evidence_kind: this.state.civilNetworkEdgeEvidenceKind, description: this.state.civilNetworkEdgeDescription, evidence: [], confidence: this.state.civilNetworkEdgeConfidence, review_state: this.state.civilNetworkEdgeReview };',
    'const evidenceKind = this.state.civilNetworkEdgeEvidenceKind; let evidence = []; if (evidenceKind === "sourced_observation") { const title = (window.prompt("Evidence title / citation", "") || "").trim(); const excerpt = (window.prompt("Evidence excerpt", "") || "").trim(); const url = (window.prompt("Public URL (optional; Cancel for bibliographic note)", "") || "").trim(); if (!title || !excerpt) { this.setState({ civilNetworkStatus: "Sourced relationships require a title and excerpt." }); return; } evidence = [{ title, url: url || null, bibliographic_note: url ? null : "Manual dashboard citation", retrieved_at: new Date().toISOString().slice(0, 10), trust_status: "needs_review", excerpt, confidence: this.state.civilNetworkEdgeConfidence, review_state: this.state.civilNetworkEdgeReview }]; } const edge = { id: "edge-" + Date.now().toString(36), from_node_id: this.state.civilNetworkEdgeFrom, to_node_id: this.state.civilNetworkEdgeTo, kind: this.state.civilNetworkEdgeKind, evidence_kind: evidenceKind, description: this.state.civilNetworkEdgeDescription, evidence, confidence: this.state.civilNetworkEdgeConfidence, review_state: this.state.civilNetworkEdgeReview };',
)
_replace_patch_new(
    "civil network: add API helpers",
    'civilNetworkMap(network) { return !network ? [] : (network.relationships || []).map((edge) => ({ from: edge.from_node_id, to: edge.to_node_id, label: edge.kind + " · " + edge.evidence_kind + " · " + edge.confidence, warning: (edge.evidence || []).some((item) => item.trust_status !== "current") })); }',
    'civilNetworkMap(network) { return !network ? [] : (network.relationships || []).map((edge) => ({ from: edge.from_node_id, to: edge.to_node_id, kind: edge.kind, evidence_kind: edge.evidence_kind, confidence: edge.confidence, review_state: edge.review_state, label: edge.kind + " · " + edge.evidence_kind + " · " + edge.confidence + " · " + edge.review_state, warning: (edge.evidence || []).some((item) => item.trust_status !== "current") })); }',
)
_replace_patch_new(
    "civil network: compute workspace bindings",
    'const civilNetworkEdges = civilNetworkMap.filter((edge) => (!civilNetworkFilters.kind || edge.label.indexOf(civilNetworkFilters.kind) >= 0) && (!civilNetworkFilters.evidence || edge.label.indexOf(civilNetworkFilters.evidence) >= 0) && (!civilNetworkFilters.confidence || edge.label.indexOf(civilNetworkFilters.confidence) >= 0) && (!civilNetworkFilters.warnings || edge.warning));',
    'const civilNetworkEdges = civilNetworkMap.filter((edge) => (!civilNetworkFilters.kind || edge.kind === civilNetworkFilters.kind) && (!civilNetworkFilters.evidence || edge.evidence_kind === civilNetworkFilters.evidence) && (!civilNetworkFilters.confidence || edge.confidence === civilNetworkFilters.confidence) && (!civilNetworkFilters.review || edge.review_state === civilNetworkFilters.review) && (!civilNetworkFilters.warnings || edge.warning));',
)
