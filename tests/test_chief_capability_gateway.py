...
def _gateway(tmp_path, repo_root) -> ChiefCapabilityGateway:
...
def test_gateway_appends_audits_and_undoes_once(tmp_path) -> None:
...
def test_gateway_summary_and_private_cadence_append(tmp_path) -> None:
    gateway = _gateway(tmp_path, tmp_path)
    response = gateway.execute(
        ChiefCapabilityRequest(
            user_key="marine",
...
def test_repository_search_is_root_scoped_and_excludes_projects(tmp_path) -> None:
