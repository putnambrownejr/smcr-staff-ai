...
def test_app_serves_dashboard_static_and_seed_data_from_non_root_cwd(
    ...
    shim_response = client.get("/static/dashboard/reveal-shim.js")
    assert shim_response.status_code == 200
    assert "javascript" in shim_response.headers["content-type"]
    assert shim_response.text.strip()
