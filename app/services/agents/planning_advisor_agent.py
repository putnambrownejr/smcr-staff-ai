...
class PlanningAdvisorAgent(Agent):
    ...
    def run(self, input_text: str, context: AgentContext) -> AgentRunResponse:
        ...
        answer = (
            ...
            "Deliberate MCPP rhythm (the six steps of MCWP 5-10):\n"
            "- 1. Problem framing: what is actually being decided, and what is still ambiguous? This is "
            "where mission analysis lives — assumptions, constraints/restraints, specified/implied/"
            "essential tasks, risk, information gaps, and the commander's initial intent and CCIRs.\n"
            "- 2. COA development: build real options, not cosmetic variants.\n"
            "- 3. COA war game: what breaks first, what support fails first, which decision point matters.\n"
            "- 4. COA comparison and decision: tell the commander what each option gives, costs, and risks.\n"
            "- 5. Orders development: publish only after the staff frame is coherent.\n"
            "- 6. Transition: hand the plan to the executors with a confirmation brief and rehearsal.\n\n"
    ...
    def _scenario_response(self, input_text: str, context: AgentContext) -> AgentRunResponse:
        ...
        template = (
            ...
            "A specific scenario was detected. Use this mission analysis framework "
            "to structure the planning effort. Work through each section using the "
            "scenario details provided.\n\n"
            "PROBLEM FRAMING / MISSION ANALYSIS (scenario-specific; MCPP step 1):\n\n"
            "1. MISSION RESTATEMENT:\n"
            "   - State the higher commander's intent as understood from the scenario\n"
            "   - Restate the mission in one sentence: who, what, when, where, why\n\n"
            "2. SPECIFIED / IMPLIED / ESSENTIAL TASKS:\n"
            "   - List specified tasks explicitly stated in the scenario or tasking\n"
            "   - Identify implied tasks required but not stated — logistics, coordination, comms\n"
            "   - Determine essential tasks that must be accomplished for mission success\n\n"
            "3. CONSTRAINTS AND LIMITATIONS:\n"
            "   - Identify constraints (must do): legal, policy, ROE, host nation requirements\n"
            "   - Identify limitations (must not do): Posse Comitatus, SOFA restrictions, "
            "political sensitivities\n\n"
            "4. ASSUMPTIONS:\n"
            "   - List assumptions that, if wrong, change the plan\n"
            "   - Identify which assumptions need validation before execution\n"
            "   - Determine the trigger to revalidate each assumption\n\n"
            "5. RISK ASSESSMENT:\n"
            "   - Identify the most likely risk to mission success\n"
            "   - Identify the most dangerous risk\n"
            "   - Recommend available mitigation\n\n"
            "6. INITIAL PLANNING TIMELINE:\n"
            "   - Identify key decision points and their suspenses\n"
            "   - Determine when staff estimates are due from: S-2, S-3, S-4, S-6, G-9\n"
            "   - Recommend commander's decision brief timing and format\n\n"
            "7. INFORMATION REQUIREMENTS:\n"
            "   - Develop PIR — intelligence gaps that affect the decision\n"
            "   - Identify FFIR — own-force status needed for planning\n"
            "   - Develop CIR — civil information requirements\n\n"
            "PLANNING WATCHPOINTS:\n"
            "- Identify which assumption is doing too much work\n"
            "- Assess where the plan will break first in execution\n"
            "- Determine which staff section is not yet integrated\n"
