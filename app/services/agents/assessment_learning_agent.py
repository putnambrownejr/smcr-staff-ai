...
class AssessmentLearningAgent(Agent):
    def __init__(self) -> None:
        self.metadata = AgentMetadata(
            ...
            system_prompt=(
                ...
                "- Individual T&R events: coded MOS-functional area-event number; 1000-level = core "
                "skills, 2000-level = core-plus (e.g., 0311-PAT-1001 pattern; pull the real code from "
                "NAVMC 3500.44D).\n"
                "- Collective T&R events: coded community-functional area-level (e.g., INF-MAN-4xxx); "
                "the thousands digit is the unit level — 3000 squad, 4000 platoon, 5000 company, "
                "6000 battalion (verify in the applicable T&R manual).\n"
                "- MET assessment ratings in DRRS-MC (MCO 3000.13B): Y (yes — trained to standard), "
                "Q (qualified yes — can perform with limitations), N (no). T/P/U is Army, not Marine Corps.\n"
                ...
                "- Training (T-level) is one of four resource areas: P (personnel), S (supply/equipment on hand), "
                "R (equipment readiness/condition), T (training). MET assessments are Y/Q/N.\n"
    ...
    def run(self, input_text: str, context: AgentContext) -> AgentRunResponse:
        ...
        answer = (
            ...
            "- What MET assessment applies in DRRS-MC terms: Y (yes), Q (qualified yes), or N (no) per MCO 3000.13B?\n"
