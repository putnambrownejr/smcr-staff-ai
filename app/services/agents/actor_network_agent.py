...
class ActorNetworkAnalystAgent(Agent):
    ...
    def __init__(self) -> None:
        self.metadata = AgentMetadata(
            ...
            description=(
                "Maps organizations, institutions, broad actor categories, and validated role-bound public leaders "
                "from a civil-network snapshot; records relationship confidence and gaps without profiling people."
            ),
            ...
            system_prompt=(
                "Map organizations, institutions, broad categories, and only validated role-bound public leaders. "
                "State evidence confidence and gaps. Do not profile private people, identify targets, or infer activity."
            ),
        )
    ...
    def run(self, input_text: str, context: AgentContext) -> AgentRunResponse:
        ...
        answer = (
            _render_answer(output, evidence, area_study).removesuffix(_DRAFT_WARNING)
    ...
    def _safety_response(self, input_text: str) -> AgentRunResponse:
        ...
        return self._response(
            ...
            scenario_output_status=ScenarioOutputStatus.validated,
            additional_warnings=[
                "Sensitive request: individual targeting and private-person profiling are not supported."
            ],
        )
...
def _actors_from_evidence(evidence: list[dict[str, str]]) -> list[dict[str, str]]:
    if not evidence:
        return [
            {
                "actor": "Organizations and institutions not yet supplied",
                "interest": "Collection gap",
                "influence": "Unknown",
                "confidence": "low",
            }
        ]
        ...
        return [
            {
                ...
                "relationship": "Unknown; validate before use.",
                "confidence": "low",
            }
        ]
    return [
        {
            "from": actors[0]["actor"],
            "to": actors[1]["actor"],
            "relationship": "Relationship not established by supplied evidence.",
            "confidence": "low",
        }
    ]
...
def _gaps(area_study: dict[str, object], evidence: list[dict[str, str]]) -> list[str]:
    gaps = [
        "Validate organization interests, capabilities, and relationships with approved public or scenario sources."
    ]
    if not area_study:
...
def _render_answer(
    output: ActorNetworkScenarioOutput, evidence: list[dict[str, str]], area_study: dict[str, object]
) -> str:
    actor_lines = "\n".join(
        f"- {item['actor']}: interest={item['interest']}; influence={item['influence']}; confidence={item['confidence']}."
        for item in output.actors
    )
    relationship_lines = "\n".join(
        f"- {item['from']} → {item['to']}: {item['relationship']} (confidence: {item['confidence']})."
        for item in output.relationships
    )
    evidence_lines = (
        "\n".join(
            f"- {item.get('title', 'Saved local source')}: {item.get('excerpt', 'No excerpt supplied.')}"
            for item in evidence
        )
        or "- No local evidence was attached or retrieved."
    )
    area_note = (
