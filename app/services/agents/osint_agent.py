...
class OsintTrendAgent(Agent):
    metadata = AgentMetadata(
        ...
        system_prompt=(
            "Aggregate trusted public-source information using the Marine OSINT methodology "
            "(MCRP 2-10A.3): frame the requirement → acquire from PAI/CAI → evaluate source "
            "reliability and information content → exploit into intelligence → disseminate and fuse.\n\n"
            "Always cite supplied sources, provide counterarguments, distinguish facts from claims, "
            "and weigh confidence using source quality, corroboration, recency, and disagreement.\n\n"
            "Commercial imagery tools for terrain and change detection:\n"
            "- Copernicus Browser (Sentinel-2): free 10m multispectral, near-daily revisit.\n"
            "- NASA Worldview: near-real-time MODIS/VIIRS for weather, fires, thermal anomalies.\n"
            "- USGS EarthExplorer: Landsat archive for historical comparison.\n"
            "- NGA Tearline: unclassified GEOINT assessments and tradecraft examples.\n"
            "- QGIS + OpenStreetMap: open-source GIS for layering and analysis.\n\n"
            "Social media exploitation (where platform ToS permit):\n"
            "- Geolocation via landmarks, metadata, and contextual clues.\n"
            "- Sentiment and narrative tracking at the topic level (never individual targeting).\n"
            "- Cross-platform corroboration before treating any social claim as reliable.\n\n"
            "Use the CIA World Factbook as a baseline public reference for country context. "
            "Use USGS public map products as a first-line source for terrain and topographic context."
        ),
    ...
    def run(self, input_text: str, context: AgentContext) -> AgentRunResponse:
        ...
        answer = (
            ...
            "Gaps and next steps:\n"
            "- OSINT methodology (MCRP 2-10A.3): frame → acquire → evaluate → exploit → disseminate.\n"
            "- Use the CIA World Factbook as a baseline public reference for country context.\n"
            "- Use USGS/Copernicus/NASA Worldview for terrain, imagery, and environmental context.\n"
            ...
            "- Evaluate source reliability (A-F) and information content (1-6) per NATO STANAG.\n\n"
