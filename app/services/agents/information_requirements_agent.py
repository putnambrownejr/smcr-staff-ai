...
class InformationRequirementsManagerAgent(Agent):
    ...
    def __init__(self) -> None:
        self.metadata = AgentMetadata(
            ...
            disallowed_inputs=[
                ...
                "real collection tasking",
                "sensitive movements or COMSEC",
                "individual targeting",
            ],
            system_prompt=(
                "Create advisory PIR, FFIR, and CIR recommendations tied to a decision. Do not issue collection tasking "
                "or claim authority. Mark missing evidence and ownership as human-review requirements."
            ),
        )
    ...
    def run(self, input_text: str, context: AgentContext) -> AgentRunResponse:
        ...
        output = InformationRequirementsScenarioOutput(
            requirements=requirements, evidence_gaps=_gaps(area_study, actor_network, evidence)
        )
        local_citations = _local_citations(evidence)
        return self._response(
            answer=_render_answer(output, decision, area_study, actor_network).removesuffix(_DRAFT_WARNING)
            ...
            follow_up_questions=[
                "What commander decision and decision deadline does each requirement support?",
                "Which approved source or scenario-control input can validate each indicator?",
                "Who will a human assign as the responsible staff owner?",
            ],
...
def _source_evidence(context: AgentContext) -> list[dict[str, str]]:
    ...
    return (
        [{str(k): str(v) for k, v in item.items() if v is not None} for item in raw if isinstance(item, dict)]
...
def _requirements(
    decision: str, area_study: dict[str, object], actor_network: dict[str, object]
) -> list[InformationRequirement]:
    area_signal = (
        "Which area-study factor or civil/infrastructure constraint would change the decision?"
        if area_study
        else "What operational-area, civil, or infrastructure factor must be validated before the decision?"
    )
    actor_signal = (
        "Which organization-level relationship or influence pattern would change the decision?"
        if actor_network
        else "Which organization-level actor or relationship must be validated before the decision?"
    )
    return [
        InformationRequirement(
            category="PIR",
            requirement=actor_signal,
            decision_supported=decision,
            indicator="Approved source or scenario evidence confirms a relevant organization-level relationship.",
            collection_question=actor_signal,
            recommended_owner="Human assignment required (S-2/G-2 lead recommendation)",
            priority="High",
        ),
        InformationRequirement(
            category="FFIR",
            requirement="What own-force readiness, access, or support constraint would prevent the selected option?",
            decision_supported=decision,
            indicator="Validated readiness, access, or support status changes the feasible option.",
            collection_question="Which readiness or support condition must be confirmed before the decision?",
            recommended_owner="Human assignment required (operations/logistics lead recommendation)",
            priority="High",
        ),
        InformationRequirement(
            category="CIR",
            requirement=area_signal,
            decision_supported=decision,
            indicator="Approved evidence confirms a civil, population, infrastructure, or institutional factor affecting the decision.",
            collection_question=area_signal,
            recommended_owner="Human assignment required (G-9/civil lead recommendation)",
            priority="Medium",
        ),
    ]
...
def _gaps(area_study: dict[str, object], actor_network: dict[str, object], evidence: list[dict[str, str]]) -> list[str]:
    gaps = [
        "Validate each requirement's indicator, responsible owner, and decision deadline before issuing any tasking."
    ]
    if not area_study:
...
def _render_answer(
    ...
    return (
        "Information Requirements Manager — advisory decision register\n\n"
