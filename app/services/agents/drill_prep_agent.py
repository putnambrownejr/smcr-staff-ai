...
class DrillPrepCalendarAgent(Agent):
    def __init__(self) -> None:
        self.metadata = AgentMetadata(
            ...
            system_prompt=(
                ...
                "Navy personnel attached to Marine units (corpsmen, chaplains, RPs):\n"
                "- Navy reservists' orders go through their NOSC via NROWS, NOT MROWS.\n"
                "- The Marine unit is the gaining command — you request, the NOSC executes.\n"
                "- AT/ADT request flow: unit OpsO/S-1 writes request letter → NOSC submits "
                "in NROWS → CNRFC approves. This chain is slower than MROWS.\n"
                "- Start NROWS requests at T-60 minimum (vs T-45 for Marine MROWS).\n"
                "- Last-minute orders for Navy personnel are extremely difficult — the NOSC "
                "approval chain adds time the Marine system does not require.\n"
                "- Pay issues for Navy personnel route through the NOSC, not the Marine S-1.\n"
                "- If you have corpsmen or chaplains, maintain a separate tracker with: "
                "name, rate, NOSC assignment, NROWS status, and order requirement dates."
            ),
        )
    ...
    def run(self, input_text: str, context: AgentContext) -> AgentRunResponse:
        answer = (
            ...
            "- Navy personnel (corpsmen/chaplains): NROWS orders submitted? (T-60 lead time)\n\n"
            "During drill:\n"
